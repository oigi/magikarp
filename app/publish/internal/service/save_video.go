...
import (
	...
	alioss "github.com/oigi/Magikarp/pkg/oss"
	"time"
)
...
func SavePublish(ctx context.Context, req *publish.CreateVideoRequest) error {
	...
	// 存oss
	OSSClient, _ := alioss.InitOss()

	if err := OSSClient.Put(
		"video", filename, bytes.NewReader(req.Data),
	); err != nil {
		return err
	}
	...
	// 3. 写入数据到Mongo
	err = dao.NewMongoClient(ctx).InsertVideoInMongo(id, req.ActorId, req.Title, playUrl, coverUrl, req.Label, req.Category, timestamp)
	if err != nil {
		return err
	}
	return nil
